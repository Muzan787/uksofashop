...
export default function AddProductForm({ categories }: { categories: Category[] }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 bg-white p-5 lg:p-8 rounded-2xl shadow-sm border border-stone-200">
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <select name="origin" defaultValue="unspecified" className="w-full p-3.5 bg-stone-50 border border-stone-200 rounded-xl focus:ring-2 focus:ring-orange-500 outline-none font-medium">
          ...
          <div>
            <label className="flex items-start gap-3 p-3.5 bg-stone-50 border border-stone-200 rounded-xl cursor-pointer hover:border-orange-300 transition">
              <input type="checkbox" name="customMade" value="true"  className="mt-0.5 w-5 h-5 accent-orange-500 shrink-0 cursor-pointer" />
              <span>
        ...
        <div>
          ...
          <textarea name="description" rows={4} required className="w-full p-3.5 bg-stone-50 border border-stone-200 rounded-xl focus:ring-2 focus:ring-orange-500 outline-none font-medium"></textarea>
        </div>
      </div>
            ...
            <div key={index} className="flex flex-col gap-4 p-5 bg-stone-50 rounded-2xl border border-stone-200 relative">
              ...
              <div className="relative mt-2">
                ...
                <div className={`flex items-center justify-center gap-3 p-4 rounded-xl border-2 border-dashed transition ${variant.image_url ? 'border-green-500 bg-green-50' : 'border-stone-300 bg-white hover:border-orange-500'}`}>
                  {variant.isUploading ? <div className="flex items-center gap-2 text-stone-500 font-medium"><Loader2 className="w-5 h-5 animate-spin" /> Uploading...</div> : variant.image_url ? <><img src={variant.image_url} alt="Variant" className="w-12 h-12 object-cover rounded-lg shadow-sm" /><span className="text-green-700 font-bold text-sm">Image Uploaded (Tap to change)</span></> : <><ImagePlus className="w-6 h-6 text-stone-400" /><span className="font-bold text-stone-600 text-sm">Tap to Upload Photo</span></>}
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="pt-8">
        <button type="submit" disabled={isPending || isUploadingGallery || variants.some(v => v.isUploading)} className="w-full bg-orange-500 text-white py-4 rounded-2xl font-black text-lg hover:bg-orange-600 transition shadow-lg disabled:opacity-70 flex justify-center items-center gap-2 active:scale-[0.98]">
