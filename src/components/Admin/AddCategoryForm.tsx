...
export default function AddCategoryForm() {
  ...
  return (
    <form id="category-form" action={handleSubmit} className="space-y-6 bg-white p-6 lg:p-8 rounded-md shadow-sm border border-stone-200">
      <h2 className="text-xl font-bold text-stone-900 tracking-tight">Add New Category</h2>
      
      {error && <div className="p-4 bg-red-50 text-red-600 font-medium rounded-sm text-sm">{error}</div>}

      <div className="space-y-4">
        <div>
          <label className="block text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">Category Name</label>
          <input type="text" name="name" required className="w-full p-3.5 bg-stone-50 border border-stone-200 rounded-sm focus:ring-2 focus:ring-orange-500 outline-none font-medium" placeholder="e.g. Corner Sofas" />
        </div>
        <div>
          <label className="block text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">URL Slug</label>
          <input type="text" name="slug" required className="w-full p-3.5 bg-stone-50 border border-stone-200 rounded-sm focus:ring-2 focus:ring-orange-500 outline-none font-medium" placeholder="e.g. corner-sofas" />
        </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <div className={`flex items-center justify-center gap-3 p-4 rounded-sm border-2 border-dashed transition ${imageUrl ? 'border-green-500 bg-green-50' : 'border-stone-300 bg-stone-50 hover:border-orange-500'}`}>
              {isUploading ? (
      ...
      <button type="submit" disabled={isPending || isUploading} className="w-full bg-stone-900 text-white py-3.5 rounded-sm font-bold hover:bg-stone-800 transition shadow-md disabled:opacity-70 flex justify-center items-center gap-2 active:scale-[0.98]">
