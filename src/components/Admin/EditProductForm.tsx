...
export default function EditProductForm({ product, categories }: { product: Product, categories: Category[] }) {
  ...
  const [subgroupLabel, setSubgroupLabel] = useState(product.subgroup_label || '')
  ...
  const handleCreateGroup = async () => {
    ...
    const { data, error } = await supabase.from('variant_groups').insert({ name: newGroupName.trim(), slug }).select('id, name, subgroup_title').single()
    if (data) {
    ...
    <form onSubmit={handleSubmit} className="space-y-8 bg-white p-5 lg:p-8 rounded-md shadow-sm border border-stone-200">
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            <label className="flex items-start gap-3 p-3.5 bg-stone-50 border border-stone-200 rounded-sm cursor-pointer hover:border-orange-300 transition">
              <input type="checkbox" name="isFeatured" value="true" defaultChecked={product.is_featured ?? false} className="mt-0.5 w-5 h-5 accent-orange-500 shrink-0 cursor-pointer" />
              <span>
